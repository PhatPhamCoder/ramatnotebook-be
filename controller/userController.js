...
const forgotPasswordToken = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        await user.save();
        const resetURL = `Hi, Please follow this link to reset Your Password. This link is valid still 10 minutes from now,<a href='http://localhost:5000/api/user/reset-password/${token}'>CLick here</a>`
        const data = {
            to: email,
            text: "Hey user",
...
const emptyCart = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        constcart = await Cart.findOneAndRemove({ orderby: user._id });
        res.json(user);
    } catch (error) {
        throw new Error(error);
    }
...
const createOrder = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        let finalAmount = 0;
        if (couponApplied > 0 && userCart.totalAfterDiscount) {
